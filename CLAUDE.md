...
- `admin` module added (`src/admin/`) — a single dashboard-stats endpoint to replace the panel's previous pattern of firing several `take=1` requests against separate list endpoints just to read `page.total`. `GET /admin/stats` (`AuthGuard, RolesGuard` + `@Roles(Role.Admin)`) runs plain `prisma.<model>.count()`/`count({ where })` calls in parallel (`Promise.all`, no `groupBy`, no joins) and returns `{ businesses: {total,pending,approved,rejected}, reviews: {pending}, feedback: {total,unread}, businessReports: {total,unread,pending}, users: {total} }`. No new DTO (no query params). Not registered ahead of any other module in `app.module.ts` — `admin` isn't a prefix any other controller uses, so there's no `BusinessEventModule`-style ordering footgun here.
